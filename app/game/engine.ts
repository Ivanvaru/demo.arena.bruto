...
export type Outcome="hit"|"miss"|"dodge"|"block"|"perfect-block";
...
export const CLASS_TEMPLATES:Record<FighterClass,{stats:Stats;title:string}>={
  ...
  Aventurero:{title:"El imprevisible",stats:{strength:3,speed:4,agility:4,resistance:3,precision:5,luck:5}},
...
export function maxHp(f:FighterProfile){return Math.round(76+f.level*2.2+f.stats.resistance*6.2)}
...
export function simulateBattle(left:FighterProfile,right:FighterProfile,seed=(Date.now()^Math.floor(Math.random()*2**31))>>>0):BattleResult{
  ...
  const misses=[0,0],stunned=[0,0],turns:BattleTurn[]=[];
  let guard=0;
  ...
  const resolve=(actor:0|1,target:0|1,forcedCounter=false)=>{
    ...
    const hitChance=clamp(.72+(as.precision-ds.agility)*.025+misses[actor]*.09,.2,.96);
    let outcome:Outcome="hit",damage=0,hits=1,critical=false,knockback=false,knockdown=false,stun=false,counter=false;
    if(rng()>hitChance){outcome="miss";misses[actor]++}
    else{
      misses[actor]=0;
      const dodge=clamp(.035+(ds.agility-as.precision)*.013,.015,.24);
      const block=clamp(.07+(ds.resistance-as.strength)*.011,.025,.23);
      if(rng()<dodge)outcome="dodge";
      else if(rng()<block){outcome=rng()<clamp(.012+ds.luck*.004,.012,.06)?"perfect-block":"block"}
      if(outcome==="hit"||outcome==="block"){
        ...
        damage=Math.max(1,Math.round(power*1.27*lateFightBoost*(1+as.strength*.055)*variance*hits*(critical?1.62:1)));
    ...
    next[actor]+=costs[attack]/(1+as.speed*.085)+(forcedCounter?18:0);
