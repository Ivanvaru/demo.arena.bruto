...
/** The supplied PNGs act as alpha masks. Each silhouette uses its own uniform
 * scale and rig-canvas anchor so its face opening meets the base head cleanly. */
const HAIR_ARTWORK_LAYOUT={
  "mohawk-punk":{asset:"/characters/hair/puntas-salvajes.png",x:116,y:-42,size:744},
  "melena-lateral":{asset:"/characters/hair/flequillo-salvaje.png",x:148,y:-36,size:682},
} as const;
...
function TournamentBracketLines(){
  return <svg className="bracket-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
    <g>
      <path d="M12.3 12.5H14.8V25H17.3M12.3 37.5H14.8V25M12.3 62.5H14.8V75H17.3M12.3 87.5H14.8V75M27.4 25H29.4V50H31.4M27.4 75H29.4V50M41.6 50H44.9" />
      <path d="M87.7 12.5H85.2V25H82.7M87.7 37.5H85.2V25M87.7 62.5H85.2V75H82.7M87.7 87.5H85.2V75M72.6 25H70.6V50H68.6M72.6 75H70.6V50M58.4 50H55.1" />
    </g>
  </svg>;
}
...
export default function Home(){
  ...
  const saveCharacter=useCallback(async(overrides?:Partial<{level:number;xp:number;wins:number;losses:number}>)=>{
    try{
      await fetch("/api/character",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:username??"Ragnar",className:playerClass,level:overrides?.level??level,xp:overrides?.xp??xp,wins:overrides?.wins??wins,losses:overrides?.losses??losses,hairStyle:outfit.hairStyle,hairColor:outfit.hairColor,skinToneId:identity.skinToneId,eyeStyleId:identity.eyeStyleId,eyeColor:identity.eyeColor,eyebrowStyleId:identity.eyebrowStyleId})});
  ...
  const playerName=username??"Ragnar";
