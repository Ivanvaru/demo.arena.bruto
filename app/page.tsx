...
function SkeletonCharacterRig({outfit}:{outfit?:Outfit}){
  const instance=useId().replace(/[^a-zA-Z0-9_-]/g,"");
...
function Brute({side,variant,attacking,attackKind,hit,critical,reaction,defeated,control,outfit}:{side:"left"|"right";variant:"ragnar"|"brakka";attacking:boolean;attackKind:AttackKind|null;hit:boolean;critical:boolean;reaction:DefenseReaction|null;defeated:boolean;control?:MotionControl;outfit?:Outfit}){
  ...
  return <div ref={rootRef} data-motion={motion} className={`fighter motion-driven character-clean ${side} ${variant} ${attacking?`attack attack-${attackKind}`:""} ${critical?"critical":""} ${hit?"hit":""} ${reaction?`defense-${reaction}`:""} ${defeated?"defeated":""}`} aria-hidden="true"><div className="fighter-aura"/><div className="shadow"/><SkeletonCharacterRig outfit={outfit}/></div>;
}
