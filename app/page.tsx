...
export default function Home(){
  ...
  if(view==="login")return <main><section className="flow-screen login-screen"><div className="screen-heading"><small>ENTRADA A LA ARENA</small><h2>{authMode==="register"?"Crea tu cuenta":"Inicia sesión"}</h2><p>{authMode==="register"?"Tu bruto se guardará con este usuario y contraseña. Aún no usamos correo.":"Accede para recuperar tu bruto guardado."}</p></div><form className="login-form" onSubmit={event=>{event.preventDefault();if(!authBusy)submitAuth()}}><input className="login-input" type="text" maxLength={20} placeholder="Usuario" value={authUser} onChange={event=>setAuthUser(event.target.value)} autoFocus/><input className="login-input" type="password" maxLength={72} placeholder="Contraseña" value={authPass} onChange={event=>setAuthPass(event.target.value)}/>{authError?<p className="auth-error">{authError}</p>:null}<button className="primary-game-button" type="submit" disabled={authBusy||!authUser.trim()||!authPass}><span>✓</span> {authBusy?"UN MOMENTO…":authMode==="register"?"CREAR CUENTA":"ENTRAR"}</button><button className="secondary-game-button auth-switch" type="button" onClick={()=>{setAuthMode(mode=>mode==="register"?"login":"register");setAuthError(null)}}>{authMode==="register"?"¿Ya tienes cuenta? Inicia sesión":"¿Sin cuenta? Crear una"}</button></form></section></main>;
  ...
  if(view==="locker"){
    ...
    return <main><GameHeader/><section className="flow-screen locker-screen"><div className="screen-heading"><small>VESTUARIO · {username}</small><h2>Tu bruto está preparado</h2><p>Consulta sus atributos antes de entrar en la arena.</p></div><div className="locker-layout"><div className="locker-character"><Brute side="left" variant="ragnar" attacking={false} attackKind={null} hit={false} critical={false} reaction={null} defeated={false} outfit={outfit} face={face} bodyTexture={playerBodyTexture}/></div><div className="locker-sheet"><div className="sheet-title"><span>NV. {level}</span><div><h3>{player.name}</h3><p>{player.className} · {player.title}</p></div></div><div className="xp-row"><div className="xp-bar"><i style={{width:`${Math.min(100,xp/xpNeeded*100)}%`}}/></div><span>{xp} / {xpNeeded} XP</span></div><p className="record-line">{wins}V — {losses}D</p><StatsPanel fighter={player}/><div className="ability-callout"><b>✦ {CLASS_ABILITIES[playerClass].name}</b><span>{CLASS_ABILITIES[playerClass].description}</span></div><div className="class-picker" aria-label="Clase del jugador">{CLASS_NAMES.map(name=><button key={name} className={name===playerClass?"selected":""} onClick={()=>selectClass(name)}>{name}</button>)}</div></div></div><div className="locker-actions"><button className="secondary-game-button" onClick={()=>setView("creator")}>🎭 PERSONAJE</button><button className="secondary-game-button" onClick={()=>setView("lab")}>⚙ LABORATORIO</button><button className="primary-game-button" onClick={prepareEncounter}><span>⚔</span> LUCHAR</button></div><button className="logout-link" onClick={logout} type="button">Cerrar sesión</button></section></main>;
  }
