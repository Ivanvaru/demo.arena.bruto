...
function GameHeader(){return <header className="topbar"><button className="brand brand-button" aria-label="Liga de Brutos"><span>LB</span><div><small>COMBATES AUTOMÁTICOS</small><h1>Liga de Brutos</h1></div></button><div className="top-actions"><span className="season">TEMPORADA I</span></div></header>}
...
export default function Home(){
  ...
  if(view==="splash")return <main className="splash-screen"><div className="splash-logo"><span>LB</span><small>EL COMBATE COMIENZA</small><h1>LIGA DE<br/>BRUTOS</h1><p>Forja tu leyenda. La arena decidirá el resto.</p></div><button className={`start-button ${loading?"loading":""}`} onClick={enterGame} disabled={loading}>{loading?<><i style={{width:`${progress}%`}}/><b>{progress}%</b></>:<><span>⚔</span> EMPEZAR</>}</button></main>;
