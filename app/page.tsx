...
function useMotionController(rootRef:RefObject<HTMLDivElement|null>,motion:MotionName,side:"left"|"right",control?:MotionControl){
  ...
  useEffect(()=>{const root=rootRef.current;if(!root)return;const definition=MOTIONS[motion],targets=new Map<string,HTMLElement|SVGElement>();for(const motionTrack of definition.tracks){const target=motionTrack.selector==="root"?root.querySelector(".skeleton-character-rig"):root.querySelector(motionTrack.selector);if(target)targets.set(motionTrack.selector,target as HTMLElement|SVGElement)}
    const reset=()=>{root.querySelectorAll<HTMLElement|SVGElement>(".skeleton-character-rig,.skeleton-bone").forEach(element=>{element.style.transform=""})};reset();
    ...
    const tick=(now:number)=>{const delta=now-last;last=now;if(!pausedRef.current)elapsed+=delta*speedRef.current;let progress=elapsed/definition.duration;if(definition.loop){progress%=1;elapsed%=definition.duration}else progress=Math.min(1,progress);render(progress);if(definition.loop||progress<1||pausedRef.current)frame=requestAnimationFrame(tick)};render(elapsed/definition.duration);frame=requestAnimationFrame(tick);return()=>{cancelAnimationFrame(frame);reset()};
  },[rootRef,motion,side,control?.restart,control?.seek]);
}
function Brute({side,variant,attacking,attackKind,hit,critical,reaction,defeated,control}:{side:"left"|"right";variant:"ragnar"|"brakka";attacking:boolean;attackKind:AttackKind|null;hit:boolean;critical:boolean;reaction:DefenseReaction|null;defeated:boolean;control?:MotionControl}){
  const rootRef=useRef<HTMLDivElement>(null);const motion:MotionName=defeated?"defeat":reaction??(hit?"hit":attacking?(critical?"critical":attackKind??"punch"):"idle");useMotionController(rootRef,motion,side,control);
  return <div ref={rootRef} data-motion={motion} className={`fighter motion-driven character-clean ${side} ${variant} ${attacking?`attack attack-${attackKind}`:""} ${critical?"critical":""} ${hit?"hit":""} ${reaction?`defense-${reaction}`:""} ${defeated?"defeated":""}`} aria-hidden="true"><div className="fighter-aura"/><div className="shadow"/><SkeletonCharacterRig/></div>;
}
...
export default function Home(){
  ...
  if(view==="lab"){
    ...
    const speed=labSpeed==="normal"?1:labSpeed==="half"?.5:.25,phase=labProgress<MOTIONS[labMotion].contact/MOTIONS[labMotion].duration?"PREPARACIÓN":labProgress<.72?"CONTACTO":"RECUPERACIÓN";
    return <main><GameHeader/><section className="flow-screen lab-screen"><div className="screen-heading"><small>LABORATORIO DE ANIMACIONES · MOTOR JS</small><h2>Banco de pruebas del rig</h2><p>Un único reloj controla las 43 capas, el contacto y la recuperación.</p></div><div className="lab-timeline-status"><b>{phase}</b><span>{Math.round(labProgress*MOTIONS[labMotion].duration)} / {MOTIONS[labMotion].duration} ms</span></div><div className="lab-demo"><Brute side="left" variant="ragnar" attacking={attacking} attackKind={attacking?(labMotion==="kick"?"kick":"punch"):null} hit={labMotion==="hit"} critical={labMotion==="critical"} reaction={reaction} defeated={labMotion==="defeat"} control={{speed,paused:!labPlaying,restart:labKey,seek:labSeek,onProgress:setLabProgress}}/><div className="lab-target"><span>OBJETIVO</span></div></div><div className="lab-controls"><label className="lab-scrubber"><span>0</span><input aria-label="Línea temporal" type="range" min="0" max="1000" value={Math.round(labProgress*1000)} onChange={event=>{const value=Number(event.target.value)/1000;setLabPlaying(false);setLabSeek(value);setLabProgress(value)}}/><span>100%</span></label><div className="lab-motion-list">{([['idle','Guardia'],['punch','Puñetazo'],['kick','Patada'],['block','Bloqueo'],['perfect-block','Bloqueo perfecto'],['dodge','Esquiva'],['hit','Impacto'],['critical','Crítico'],['defeat','Derrota']] as const).map(([id,label])=><button key={id} className={labMotion===id?"selected":""} onClick={()=>{setLabMotion(id);setLabPlaying(true);setLabSeek(0);setLabProgress(0);setLabKey(value=>value+1)}}>{label}</button>)}</div><div className="lab-transport"><button onClick={()=>setLabPlaying(value=>!value)}>{labPlaying?"⏸ PAUSAR":"▶ REANUDAR"}</button><button onClick={()=>{setLabPlaying(true);setLabSeek(0);setLabProgress(0);setLabKey(value=>value+1)}}>↻ REPETIR</button><button onClick={()=>{const value=Math.max(0,labProgress-1/60);setLabPlaying(false);setLabSeek(value);setLabProgress(value)}}>−1 FOTOGRAMA</button><button onClick={()=>{const value=Math.min(1,labProgress+1/60);setLabPlaying(false);setLabSeek(value);setLabProgress(value)}}>+1 FOTOGRAMA</button><label>VELOCIDAD <select value={labSpeed} onChange={event=>setLabSpeed(event.target.value as "normal"|"half"|"quarter")}><option value="normal">1×</option><option value="half">0,5×</option><option value="quarter">0,25×</option></select></label></div></div><button className="secondary-game-button lab-back" onClick={()=>setView("locker")}>← VOLVER AL VESTUARIO</button></section></main>;
  }
  return <main>
    <GameHeader/>
