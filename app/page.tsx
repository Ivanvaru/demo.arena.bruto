...
import {DEFAULT_OUTFIT,loadOutfit,saveOutfit,HAIR_STYLES,HAIR_COLORS,SHIRT_COLORS,PANTS_COLORS,GLOVES_COLORS,BOOTS_COLORS,type Outfit} from "./game/wardrobe";
import {DEFAULT_IDENTITY,loadIdentity,saveIdentity,SKIN_TONES,DEFAULT_SKIN_TONE_ID,EYE_STYLES,EYE_COLORS,EYEBROW_STYLES,CLASS_CLOTHING,FACE_PLACEHOLDER_GEOMETRY,resolveFaceAppearance,type PlayerIdentity,type FaceAppearance} from "./game/character";
...
function SkeletonCharacterRig({outfit,face}:{outfit?:Outfit;face?:FaceAppearance}){
  const instance=useId().replace(/[^a-zA-Z0-9_-]/g,"");
  const baseId=`skeleton-texture-${instance}`;
  const part=(id:string)=><RigImage id={id} instance={instance} baseId={baseId}/>;
  const shirtColor=outfit?.shirtColor??null;
  const hairColor=outfit&&outfit.hairStyle!=="ninguno"?outfit.hairColor:null;
  return <div className="skeleton-character-rig" data-engine="skeletal-v1" data-layer-count="43"><svg className="character-rig-svg" viewBox={`0 0 ${RIG_CANVAS.width} ${RIG_CANVAS.height}`} role="presentation"><defs><image id={baseId} href="/characters/active/montaje-verificacion.png" x="0" y="0" width={RIG_CANVAS.width} height={RIG_CANVAS.height} preserveAspectRatio="none" style={face?{filter:face.skinFilter}:undefined}/>{RIG_LAYERS.map(layer=><clipPath key={layer.id} id={`skeleton-clip-${layer.id}-${instance}`} clipPathUnits="userSpaceOnUse"><rect x={layer.canvas_position.x} y={layer.canvas_position.y} width={layer.size.width} height={layer.size.height}/></clipPath>)}</defs><g className="skeleton-root">
    <LegChain side="der" instance={instance} baseId={baseId} pantsColor={outfit?.pantsColor} bootsColor={outfit?.bootsColor}/>
    <Bone name="core" pivot={limbPivot("pelvis")}>
      {part("pelvis")}{part("abdomen_inferior")}{part("abdomen_superior")}{part("caja_toracica")}
      <ArmChain side="der" instance={instance} baseId={baseId} glovesColor={outfit?.glovesColor}/>
      {part("pecho_superior")}
      {shirtColor?<>
        <RigColorPatch id="abdomen_inferior" instance={instance} color={shirtColor} inset={6}/>
        <RigColorPatch id="abdomen_superior" instance={instance} color={shirtColor} inset={6}/>
        <RigColorPatch id="caja_toracica" instance={instance} color={shirtColor} inset={6}/>
        <RigColorPatch id="pecho_superior" instance={instance} color={shirtColor} inset={6}/>
      </>:null}
      ...
      <ArmChain side="izq" instance={instance} baseId={baseId} glovesColor={outfit?.glovesColor}/>
    </Bone>
    <LegChain side="izq" instance={instance} baseId={baseId} pantsColor={outfit?.pantsColor} bootsColor={outfit?.bootsColor}/>
  </g></svg></div>;
}
...
function Brute({side,variant,attacking,attackKind,hit,critical,reaction,defeated,control,outfit,face}:{side:"left"|"right";variant:"ragnar"|"brakka";attacking:boolean;attackKind:AttackKind|null;hit:boolean;critical:boolean;reaction:DefenseReaction|null;defeated:boolean;control?:MotionControl;outfit?:Outfit;face?:FaceAppearance}){
  ...
  return <div ref={rootRef} data-motion={motion} className={`fighter motion-driven character-clean ${side} ${variant} ${attacking?`attack attack-${attackKind}`:""} ${critical?"critical":""} ${hit?"hit":""} ${reaction?`defense-${reaction}`:""} ${defeated?"defeated":""}`} aria-hidden="true"><div className="fighter-aura"/><div className="shadow"/><SkeletonCharacterRig outfit={outfit} face={face}/></div>;
}
...
export default function Home(){
  const [view,setView]=useState<"splash"|"login"|"creator"|"locker"|"encounter"|"battle"|"lab"|"wardrobe">("splash");
  ...
  const player=fresh(playerName,playerClass);
  const selectClass=useCallback((name:FighterClass)=>{setPlayerClass(name);updateOutfit(CLASS_CLOTHING[name])},[updateOutfit]);
  const enterGame=useCallback(async()=>{if(loading)return;setLoading(true);setProgress(0);for(let value=4;value<=100;value+=4){await wait(38);setProgress(value)}await wait(180);setLoginName(identity.name==="Ragnar"?"":identity.name);setView("login");setLoading(false)},[loading,identity.name]);
  const confirmLogin=useCallback(()=>{const trimmed=loginName.trim();updateIdentity({name:trimmed||"Ragnar"});setView("creator")},[loginName,updateIdentity]);
  const prepareEncounter=useCallback(()=>{const rival=pickRival(previousRival.current);previousRival.current=rival.id;setSelectedRival(rival);const rivalFighter=fresh(rival.name,rival.className);rivalFighter.title=rival.epithet;setFighters([fresh(playerName,playerClass),rivalFighter]);setWinner(null);setRound(0);setView("encounter")},[playerClass,playerName]);
  const fight=useCallback(async()=>{
    const id=++run.current;const rivalProfile=makeProfile(selectedRival.name,selectedRival.className,selectedRival.level);rivalProfile.title=selectedRival.epithet;const result=simulateBattle(makeProfile(playerName,playerClass),rivalProfile);setView("battle");setSeed(result.seed);let current:[BattleFighter,BattleFighter]=result.fighters.map(f=>({...f,hp:f.maxHp})) as [BattleFighter,BattleFighter];setFighters(current);setWinner(null);setRunning(true);setActive(null);setAttackKind(null);setHit(null);setReaction(null);setCritical(false);setDamage(null);setRound(0);setMessage(`${selectedRival.name}, ${selectedRival.className}, entra en la arena`);await wait(650);
  ...
  },[playerClass,playerName,selectedRival]);
  ...
  if(view==="login")return <main><section className="flow-screen login-screen"><div className="screen-heading"><small>ENTRADA A LA ARENA</small><h2>¿Cómo te llamas, guerrero?</h2><p>Es solo un nombre para tu partida en este dispositivo — no hay cuentas ni contraseñas.</p></div><form className="login-form" onSubmit={event=>{event.preventDefault();if(loginName.trim())confirmLogin()}}><input className="login-input" type="text" maxLength={18} placeholder="Tu nombre" value={loginName} onChange={event=>setLoginName(event.target.value)} autoFocus/><button className="primary-game-button" type="submit" disabled={!loginName.trim()}><span>✓</span> ACEPTAR</button></form></section></main>;
  if(view==="creator")return <main><GameHeader/><section className="flow-screen wardrobe-screen"><div className="screen-heading"><small>CREACIÓN DE PERSONAJE</small><h2>Da forma a tu bruto</h2><p>Elige clase, peinado, tono de piel y rasgos. La ropa inicial cambia según la clase.</p></div><div className="locker-layout"><div className="locker-character"><Brute side="left" variant="ragnar" attacking={false} attackKind={null} hit={false} critical={false} reaction={null} defeated={false} outfit={outfit} face={face}/></div><div className="locker-sheet wardrobe-controls"><div className="wardrobe-row"><span className="wardrobe-row-label">Nombre del avatar</span><input className="login-input wardrobe-name-input" type="text" maxLength={18} value={identity.name} onChange={event=>updateIdentity({name:event.target.value})}/></div><StyleButtonRow label="Clase" options={CLASS_NAMES.map(name=>({id:name,label:name}))} value={playerClass} onSelect={id=>selectClass(id as FighterClass)}/><StyleButtonRow label="Peinado" options={HAIR_STYLES} value={outfit.hairStyle} onSelect={id=>updateOutfit({hairStyle:id as Outfit["hairStyle"]})}/><ColorSwatchRow label="Color de pelo" colors={HAIR_COLORS} value={outfit.hairColor} onSelect={color=>updateOutfit({hairColor:color??outfit.hairColor})}/><SkinToneRow label="Tono de piel" tones={SKIN_TONES} value={identity.skinToneId} onSelect={id=>updateIdentity({skinToneId:id})}/><StyleButtonRow label="Ojos" options={EYE_STYLES} value={identity.eyeStyleId} onSelect={id=>updateIdentity({eyeStyleId:id})}/><ColorSwatchRow label="Color de ojos" colors={EYE_COLORS} value={identity.eyeColor} onSelect={color=>updateIdentity({eyeColor:color??identity.eyeColor})}/><StyleButtonRow label="Cejas" options={EYEBROW_STYLES} value={identity.eyebrowStyleId} onSelect={id=>updateIdentity({eyebrowStyleId:id})}/><p className="wardrobe-note">Ojos y cejas son un boceto de referencia por ahora — se sustituirán por arte definitivo más adelante.</p></div></div><div className="locker-actions"><button className="primary-game-button" onClick={()=>setView("locker")}><span>⚔</span> ENTRAR AL VESTUARIO</button></div></section></main>;
  if(view==="locker")return <main><GameHeader/><section className="flow-screen locker-screen"><div className="screen-heading"><small>VESTUARIO</small><h2>Tu bruto está preparado</h2><p>Consulta sus atributos antes de entrar en la arena.</p></div><div className="locker-layout"><div className="locker-character"><Brute side="left" variant="ragnar" attacking={false} attackKind={null} hit={false} critical={false} reaction={null} defeated={false} outfit={outfit} face={face}/></div><div className="locker-sheet"><div className="sheet-title"><span>NV. 1</span><div><h3>{player.name}</h3><p>{player.className} · {player.title}</p></div></div><StatsPanel fighter={player}/><div className="class-picker" aria-label="Clase del jugador">{CLASS_NAMES.map(name=><button key={name} className={name===playerClass?"selected":""} onClick={()=>selectClass(name)}>{name}</button>)}</div></div></div><div className="locker-actions"><button className="secondary-game-button" onClick={()=>setView("creator")}>🎭 PERSONAJE</button><button className="secondary-game-button" onClick={()=>setView("wardrobe")}>🧵 ROPA</button><button className="secondary-game-button" onClick={()=>setView("lab")}>⚙ LABORATORIO</button><button className="primary-game-button" onClick={prepareEncounter}><span>⚔</span> LUCHAR</button></div></section></main>;
  if(view==="wardrobe")return <main><GameHeader/><section className="flow-screen wardrobe-screen"><div className="screen-heading"><small>VESTUARIO · ROPA</small><h2>Elige la ropa de tu bruto</h2><p>Los cambios se guardan automáticamente en este dispositivo.</p></div><div className="locker-layout"><div className="locker-character"><Brute side="left" variant="ragnar" attacking={false} attackKind={null} hit={false} critical={false} reaction={null} defeated={false} outfit={outfit} face={face}/></div><div className="locker-sheet wardrobe-controls"><ColorSwatchRow label="Camiseta" colors={SHIRT_COLORS} value={outfit.shirtColor} onSelect={color=>updateOutfit({shirtColor:color})} allowNone noneLabel="Sin camiseta"/><ColorSwatchRow label="Pantalón" colors={PANTS_COLORS} value={outfit.pantsColor} onSelect={color=>updateOutfit({pantsColor:color})} allowNone noneLabel="Sin pantalón"/><ColorSwatchRow label="Guantes" colors={GLOVES_COLORS} value={outfit.glovesColor} onSelect={color=>updateOutfit({glovesColor:color})} allowNone noneLabel="Sin guantes"/><ColorSwatchRow label="Botas" colors={BOOTS_COLORS} value={outfit.bootsColor} onSelect={color=>updateOutfit({bootsColor:color})} allowNone noneLabel="Sin botas"/></div></div><div className="locker-actions"><button className="secondary-game-button" onClick={()=>updateOutfit(CLASS_CLOTHING[playerClass])}>↻ ROPA DE LA CLASE</button><button className="primary-game-button" onClick={()=>setView("locker")}><span>✓</span> VOLVER AL VESTUARIO</button></div></section></main>;
  if(view==="encounter")return <main><GameHeader/><section className="flow-screen encounter-screen"><div className="screen-heading"><small>PRÓXIMO ENCUENTRO</small><h2>La arena ha elegido</h2><p>Revisa el enfrentamiento antes de comenzar.</p></div><div className="matchup"><div className="match-card player-card"><div className="match-character"><Brute side="left" variant="ragnar" attacking={false} attackKind={null} hit={false} critical={false} reaction={null} defeated={false} outfit={outfit} face={face}/></div><small>TU BRUTO</small><h3>{fighters[0].name}</h3><b>{fighters[0].className}</b><p>{fighters[0].title}</p><StatsPanel fighter={fighters[0]}/></div><div className="match-versus"><span>VS</span><small>★ COMBATE 1 VS 1 ★</small></div><div className="match-card rival-card"><div className="match-character"><Brute side="right" variant="brakka" attacking={false} attackKind={null} hit={false} critical={false} reaction={null} defeated={false}/></div><small>RIVAL</small><h3>{fighters[1].name}</h3><b>{fighters[1].className}</b><p>{fighters[1].title}</p><StatsPanel fighter={fighters[1]}/></div></div><div className="encounter-actions"><button className="secondary-game-button" onClick={()=>setView("locker")}>VOLVER</button><button className="primary-game-button" onClick={fight}><span>⚔</span> EMPEZAR COMBATE</button></div></section></main>;
  if(view==="lab"){
