...
export default function Home(){
  ...
  const fight=useCallback(async()=>{
    ...
    for(let index=0;index<result.turns.length;index++){if(run.current!==id)return;const turn=result.turns[index],source=current[turn.actor],target=current[turn.target],visual:AttackKind=turn.attack==="kick"?"kick":"punch";setRound(index+1);setAttackKind(visual);setActive(turn.actor);setMessage(`${source.name} salta hacia ${target.name}`);await wait(345);if(run.current!==id)return;current=current.map((f,i)=>i===turn.target?{...f,hp:turn.hpAfter}:f) as [BattleFighter,BattleFighter];setFighters(current);setHit(turn.damage>0?turn.target:null);setDamage(turn.damage||null);setMessage(outcomeText(turn.outcome,source.name,target.name,turn.attack,turn.critical));await wait(turn.knockdown||turn.stun?555:515);setHit(null);setDamage(null);setActive(null);setAttackKind(null);if(turn.knockdown){setMessage(`${target.name} cae y debe recomponerse`);await wait(260)}else if(turn.stun){setMessage(`${target.name} queda aturdido`);await wait(180)}}
